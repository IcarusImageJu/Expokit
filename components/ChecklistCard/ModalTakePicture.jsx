...
function ModalTakePicture({
	...
	async function openCameraRoll() {
		...
		if (status === 'granted') {
			...
			const perms = await ImagePicker.requestCameraRollPermissionsAsync();
			if (perms.status === 'granted') {
				openCameraRoll();
			} else {
				Alert.alert(t('grantAccessCameraRollTitle'), t('grantAccessCameraRollMessage'),
					[
						{ text: t('Activate'), onPress: () => IntentLauncher.startActivityAsync(IntentLauncher.ACTION_MANAGE_APPLICATIONS_SETTINGS) },
					]);
			}
		}
	}
				...
				<View style={styles.modalContainer}>
					<View style={styles.modal}>
						<View style={styles.controls}>
							...
							>
								<Text style={styles.controlText}>{t('Take a picture')}</Text>
							</TouchableHighlight>
							<Separator small light />
							<TouchableHighlight
								underlayColor={EStyleSheet.value('$grey')}
			...
			>
				<View style={styles.modalView}>
					<View style={styles.imageContainer}>
						{image && <Image resizeMode="contain" style={styles.image} source={{ uri: image.uri }} /> }
					</View>
